...
export default function SessionDetailClient({ sessionId }: { sessionId: string }) {
  ...
  // Handle toast.info call in scan function
  useEffect(() => {
    if (location.pathname.includes(`/sessions/${sessionId}`)) {
      // Fix the toast.info reference in the handleScanComplete function
      const existingHandleScanComplete = handleScanComplete;
      handleScanComplete = async (barcodeData: string, method: string, imageFile?: File) => {
        try {
          await existingHandleScanComplete(barcodeData, method, imageFile);
        } catch (error) {
